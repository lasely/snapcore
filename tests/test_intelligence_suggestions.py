...
# ---------------------------------------------------------------------------
# Timestamp volatile -> SUGGEST_SANITIZER (datetime)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# UUID volatile -> SUGGEST_SANITIZER (uuid)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Generic volatile (no pattern) -> SUGGEST_JSON_MASK
# ---------------------------------------------------------------------------
...
class TestJsonMaskSuggestion:
    ...
    def test_json_mask_confidence_penalized(self):
        ...
        mask_suggestions = [s for s in suggestions if s.code == SUGGEST_JSON_MASK]
        assert len(mask_suggestions) >= 1
        # 5 runs -> raw confidence = 1.0, mask penalty -> 0.9
...
# ---------------------------------------------------------------------------
# Multiple correlated volatile siblings -> SUGGEST_RELATIONAL_SANITIZER
# ---------------------------------------------------------------------------
...
class TestRelationalSanitizer:
    ...
    def test_relational_confidence_penalized(self):
        ...
        assert len(rel_suggestions) >= 1
        # 5 runs -> raw confidence = 1.0, relational penalty -> 0.85
...
# ---------------------------------------------------------------------------
# Empty findings -> empty suggestions
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Confidence filtering (< 0.3 excluded)
# ---------------------------------------------------------------------------
...
class TestConfidenceFiltering:
    ...
    def test_sufficient_runs_high_confidence_kept(self):
        ...
        # 5 runs -> confidence should be high enough to pass
...
# ---------------------------------------------------------------------------
# Canonical sort order (confidence DESC, target_path ASC, code ASC)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Pattern priority (sanitizer over mask)
# ---------------------------------------------------------------------------
...
class TestPatternPriority:
    ...
    def test_pattern_handled_path_excluded_from_relational(self):
        ...
        # Both volatile paths are handled by pattern-based suggestions,
        # so no relational suggestion should be emitted
        assert len(rel_suggestions) == 0


# ---------------------------------------------------------------------------
# Fix #3 regression: relational volatile_field parameters preserved in sidecar
# ---------------------------------------------------------------------------
...
class TestRelationalParametersSerialization:
    ...
    def test_all_volatile_fields_preserved_in_sidecar(self):
        ...
        # All 3 fields must be preserved as a list
